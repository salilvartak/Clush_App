...
class HeartLoader extends StatefulWidget {
  ...
  const HeartLoader({super.key, this.size = 50.0, this.color});
...
class QPainter extends CustomPainter {
  ...
  QPainter({required this.drawProgress, required this.color});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double easedDraw = const Cubic(
      0.4,
      0,
      0.2,
      1,
    ).transform(strokeStepProgress);
      ...
      if (dotOpacity > 0) {
        ...
        if (t != null) {
          ...
          Paint dotPaint = Paint()
            ..color = kBlush.withOpacity(dotOpacity)
